...
void Gamepad::update_touchpad_contact_from_touchscreen(touchscreen::Touchscreen *ts, float x_min, float x_max,
                                                       float y_min, float y_max, bool &was_active, int8_t &touch_id,
                                                       uint16_t x_out_min, uint16_t x_out_max) {
  static constexpr uint16_t TOUCHPAD_Y_MAX = 1079;
  ...
  const uint16_t x = scale_touchpad_raw(static_cast<float>(tp->x), x_min, x_max, x_out_min, x_out_max);
  const uint16_t y = scale_touchpad_raw(static_cast<float>(tp->y), y_min, y_max, 0, TOUCHPAD_Y_MAX);
  if (!was_active) {
    touch_id = m_dualsense->touchpadStartTouch(x, y);
    was_active = true;
  } else {
...
#define CALL_LEFT_CONTACT(x_min_, x_max_) \
  update_touchpad_contact(m_left_touch_sensor, m_left_touch_x_sensor, m_left_touch_y_sensor, m_left_touch_x_min, \
                          ...
                          m_left_touch_id, (x_min_), (x_max_), cproto::IN_LEFT_TOUCH, cproto::IN_LEFT_TOUCH_X, \
                          cproto::IN_LEFT_TOUCH_Y)
#define CALL_RIGHT_CONTACT(x_min_, x_max_) \
  update_touchpad_contact(m_right_touch_sensor, m_right_touch_x_sensor, m_right_touch_y_sensor, m_right_touch_x_min, \
                          ...
                          m_right_touch_id, (x_min_), (x_max_), cproto::IN_RIGHT_TOUCH, cproto::IN_RIGHT_TOUCH_X, \
                          cproto::IN_RIGHT_TOUCH_Y)

  if (has_left && has_right) {
#ifdef GAMEPAD_USE_TOUCHSCREEN
    if (m_left_touchscreen_ != nullptr)
      update_touchpad_contact_from_touchscreen(m_left_touchscreen_, m_left_touch_x_min, m_left_touch_x_max,
                                               m_left_touch_y_min, m_left_touch_y_max, m_left_touch_was_active,
                                               m_left_touch_id, 0, TOUCHPAD_X_LEFT_MAX);
    else
#endif
      CALL_LEFT_CONTACT(0, TOUCHPAD_X_LEFT_MAX);
#ifdef GAMEPAD_USE_TOUCHSCREEN
    if (m_right_touchscreen_ != nullptr)
      update_touchpad_contact_from_touchscreen(m_right_touchscreen_, m_right_touch_x_min, m_right_touch_x_max,
                                               m_right_touch_y_min, m_right_touch_y_max, m_right_touch_was_active,
                                               m_right_touch_id, TOUCHPAD_X_RIGHT_MIN, TOUCHPAD_X_MAX);
    else
#endif
      CALL_RIGHT_CONTACT(TOUCHPAD_X_RIGHT_MIN, TOUCHPAD_X_MAX);
  } else if (has_left) {
#ifdef GAMEPAD_USE_TOUCHSCREEN
    if (m_left_touchscreen_ != nullptr)
      update_touchpad_contact_from_touchscreen(m_left_touchscreen_, m_left_touch_x_min, m_left_touch_x_max,
...
#endif
      CALL_LEFT_CONTACT(0, TOUCHPAD_X_MAX);
  } else if (has_right) {
#ifdef GAMEPAD_USE_TOUCHSCREEN
    if (m_right_touchscreen_ != nullptr)
      update_touchpad_contact_from_touchscreen(m_right_touchscreen_, m_right_touch_x_min, m_right_touch_x_max,
...
#endif
      CALL_RIGHT_CONTACT(0, TOUCHPAD_X_MAX);
  }

#undef CALL_LEFT_CONTACT
#undef CALL_RIGHT_CONTACT
}
